...
run_test_device_registration() {
    ...
    response=$(curl -sf -X POST \
        ...
        -H "X-Device-ID: e2e-ios-device-001" \
        -d '{"token": "fake_apns_token_e2e", "platform": "ios"}' \
    ...
    curl -sf -X POST \
        -H "Content-Type: application/json" \
        -H "X-Device-ID: e2e-ios-device-001" \
        -d '{"token": "updated_apns_token_e2e", "platform": "ios"}' \
    ...
    bad_response=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
        -H "Content-Type: application/json" \
        -H "X-Device-ID: e2e-ios-device-002" \
        -d '{"token": "", "platform": "ios"}' \
        "http://localhost:$E2E_SERVER_PORT/api/v1/devices" 2>/dev/null)
