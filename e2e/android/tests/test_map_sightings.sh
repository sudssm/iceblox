#!/bin/bash
# Test: Map sightings API endpoint + Android UI navigation
...
run_test_map_sightings_validation() {
    ...
    http_code=$(curl -s -o /dev/null -w "%{http_code}" -X POST \
        "http://localhost:$E2E_SERVER_PORT/api/v1/map-sightings?lat=40&lng=-74&radius=10" 2>/dev/null)
    if [ "$http_code" = "405" ]; then
        echo "  PASS: POST method returns 405"
        E2E_PASS=$((E2E_PASS + 1))
    else
        echo "  FAIL: POST method should return 405 (got $http_code)"
        E2E_FAIL=$((E2E_FAIL + 1))
    fi
...
run_test_map_view_ui() {
    echo ""
    echo "=== TEST: Map View UI ==="
    echo ""

    grant_permissions
    launch_app

    if tap_button_by_text "View Map"; then
        echo "  PASS: Tapped 'View Map' button"
        E2E_PASS=$((E2E_PASS + 1))
    else
        echo "  FAIL: Could not find 'View Map' button"
        E2E_FAIL=$((E2E_FAIL + 1))
        echo "=== END: Map View UI ==="
        return
    fi

    sleep "$E2E_SETTLE_WAIT"

    if wait_for_ui_text "Reported ICE vehicles near you" 10; then
        echo "  PASS: Map header text visible"
        E2E_PASS=$((E2E_PASS + 1))
    else
        echo "  FAIL: Map header text not found"
        E2E_FAIL=$((E2E_FAIL + 1))
    fi
    ...
    if wait_for_ui_text "Start Camera" 5; then
        echo "  PASS: Returned to splash screen"
        E2E_PASS=$((E2E_PASS + 1))
    else
        echo "  FAIL: Did not return to splash screen"
        E2E_FAIL=$((E2E_FAIL + 1))
    fi
