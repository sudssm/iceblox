...
| **Background capture** | Android background capture runs in a `LifecycleService` foreground service with `foregroundServiceType="camera"`. The activity starts the service in `onPause` (when it loses foreground) and stops it in `onResume` (when it regains foreground), so CameraX preview can rebind cleanly without competing for the camera. Using `onPause`/`onResume` rather than `onStart`/`onStop` ensures the handoff triggers as soon as the activity leaves the foreground (e.g., notification shade pulled down, recent apps opened), preventing any gap in capture. |
