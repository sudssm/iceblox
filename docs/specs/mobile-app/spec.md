...
When the app is opened, it MUST display a splash screen with the app name, a "Start Camera" button, a "View Map" button, and a "Report ICE Activity" button (REQ-M-60). Each button MUST display a leading icon alongside its text label: a camera icon for "Start Camera" (iOS: `camera.fill`, Android: `Videocam`), a map icon for "View Map" (iOS: `map.fill`, Android: `Map`), and a megaphone icon for "Report ICE Activity" (iOS: `megaphone.fill`, Android: `Campaign`). The buttons MUST use a fixed width (not minimum width) for consistent alignment. The top-right corner MUST display three icon buttons: a yellow question-mark icon for Help (REQ-M-71), a white gear icon for Settings (REQ-M-70), and a green chat-bubble icon for Contact (REQ-M-72). Camera capture and plate detection MUST begin when the user taps "Start Camera". Tapping "Report ICE Activity" MUST open the ICE vehicle report form (REQ-M-61). This provides an explicit user-initiated start rather than immediately activating the camera on launch.
